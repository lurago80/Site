...
**Pendência conhecida (módulo de pagamento):** o checkout público já cria a cobrança Pix real quando há gateway ativo, mas o pagamento por **cartão online** ainda não tem tokenização no front-end (exigiria Mercado Pago.js/Bricks do lado do cliente) — hoje `forma_pagamento = 'cartao'` na loja pública marca a venda como paga sem cobrança real; e o PDV físico ainda não expõe seletor de forma de pagamento na tela do caixa (`resources/views/pdv/caixa.blade.php`), embora o backend já aceite e persista `forma_pagamento_id`.
